...
import type { WaitTimeSpeciesPoint } from '../types';
import { formatQueryError } from '../lib/utils';
import { useFullscreenFrame } from './FullscreenFrame';
import { chartTheme } from '../lib/chartTheme';

interface WaitTimeSpeciesChartProps {
    data?: WaitTimeSpeciesPoint[];
    loading: boolean;
    error?: Error | null;
}
...
export const WaitTimeSpeciesChart = ({ data, loading, error }: WaitTimeSpeciesChartProps) => {
        ...
        return (
            <div className={`${chartHeightClass} w-full bg-brand-card border border-semantic-danger/30 rounded-xl flex items-center justify-center px-4 text-center`}>
                <p className="text-sm text-semantic-danger">
                    Could not load wait time by species: {formatQueryError(error)}
                </p>
            </div>
        );
    }
    ...
    if (!data || data.length === 0) {
        return (
            <div className={`${chartHeightClass} w-full bg-brand-card border border-subtle rounded-xl flex items-center justify-center text-fg-muted text-sm sm:text-base`}>
                No wait time by species data available
            </div>
        );
    }
    ...
    return (
        <div className="bg-brand-card border border-subtle rounded-xl p-4 sm:p-6 shadow-card">
            <div className="mb-4 sm:mb-6">
                <h3 className="text-base sm:text-lg font-medium text-fg-primary">Average Wait Time by Species</h3>
                <p className="text-xs text-fg-muted mt-1">Average claim delay using consult date/time when present.</p>
            </div>
