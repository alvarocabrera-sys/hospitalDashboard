...
// netlify/lib/hospitalConsultTable.ts
var CONSULT_FACT_HOSPITAL_TABLE = "consult_fact_hospital";
var ensureHospitalConsultTable = async (client) => {
  await client.query(`
...
async function getLegacyConsultFactClaimColumn(client) {
...
async function backfillHospitalFromLegacy(client) {
...
var HOSPITAL_INGESTION_CHECKPOINT_TABLE = "hospital_ingestion_checkpoint";
var ensureIngestionCheckpointTable = async (client) => {
  await client.query(`
...
var getIncrementalAfterIsoForIngestion = async (client, forceFullSync) => {
  if (forceFullSync) {
    return null;
  }
...
var refreshIngestionCheckpointFromFact = async (client) => {
  await ensureIngestionCheckpointTable(client);
  await client.query(`
        INSERT INTO ${HOSPITAL_INGESTION_CHECKPOINT_TABLE} (id, last_sync_at)
        SELECT
...
// netlify/lib/internalConsultHospital.ts
var INTERNAL_CONSULT_HOSPITAL_CODE = "PRICESMART";
var INTERNAL_CONSULT_HOSPITAL_NAME = "PriceSmart";

// netlify/lib/ingestion.ts
var resolvedInternalConsultHospital = () => ({
  code: INTERNAL_CONSULT_HOSPITAL_CODE,
  internalName: INTERNAL_CONSULT_HOSPITAL_NAME,
  premiumTier: null,
  unknownReason: null
});
var getBubbleHospitalInternalNameField = () => process.env.BUBBLE_HOSPITAL_INTERNAL_NAME_FIELD ?? "Internal Name";
var pickInternalNameFromHospital = (h) => {
...
var pickPremiumTier = (h) => {
  const a = h["Premium Tier"];
  const b = h["OS_Premium Tier"];
  const s = (typeof a === "string" && a.trim() !== "" ? a.trim() : void 0) ?? (typeof b === "string" && b.trim() !== "" ? b.trim() : void 0);
  return s ?? null;
};
var getBubbleApiBaseUrl = () => (process.env.BUBBLE_API_BASE_URL ?? "https://app.getvetwise.com/api/1.1").replace(/\/+$/, "");
var getBubbleObjectsUrl = () => `${getBubbleApiBaseUrl()}/obj`;
var getBubbleConsultationUrl = () => `${getBubbleObjectsUrl()}/Consultation`;
var getConstraintConfig = () => ({
...
var buildBaseConsultationConstraints = (config) => [
...
var getConstraintProbeSteps = (config, lastModified) => {
  const steps = [];
...
var logConstraintProbeFailure = async (bubbleApiUrl, bubbleToken, config, lastModified, sortFieldForProbe) => {
  ...
  for (const step of getConstraintProbeSteps(config, lastModified)) {
    try {
      await axios_default.get(bubbleApiUrl, {
        params: {
...
var getMonthBucket = (dateStr) => {
  const d = new Date(dateStr);
  const yyyy = d.getFullYear();
  const mm = String(d.getMonth() + 1).padStart(2, "0");
  return `${yyyy}-${mm}`;
};
var parseRegion = (region) => {
...
var getClaimedAtValue = (item) => {
  const candidate = item["Claimed at"] ?? item["Claimed At"] ?? item["Claimed Date"] ?? item["Claimed date"];
...
var getConsultDateTimeValue = (item) => {
  const candidate = item["Consult Date / Time"] ?? item["Consult Date/Time"] ?? item["Consult Date Time"] ?? item["Consult Date"];
  if (typeof candidate !== "string") {
    return null;
  }
  const trimmed = candidate.trim();
...
var fetchAllPages = async (token, objectName, constraints = []) => {
  let allResults = [];
  let cursor = 0;
  let remaining = 1;
  ...
  while (remaining > 0) {
    try {
      const response = await axios_default.get(`${bubbleObjectsUrl}/${objectName}`, {
        params: {
...
var runIngestion = async (bubbleToken, dbUrl, maxDurationMs = 2e4, options = {}) => {
  ...
  const bubbleObjectsUrl = getBubbleObjectsUrl();
  const hospitalCache = /* @__PURE__ */ new Map();
  const resolveHospital = async (hospitalBubbleId) => {
    ...
    try {
      const res = await axios_default.get(`${bubbleObjectsUrl}/Hospital/${id}`, {
        headers: { Authorization: `Bearer ${bubbleToken}` }
      });
      const h = res.data.response;
      const code = (typeof h.Code === "string" && h.Code.trim() !== "" ? h.Code.trim() : void 0) ?? (typeof h["#"] === "number" ? String(h["#"]) : void 0) ?? (typeof h.Name === "string" && h.Name.trim() !== "" ? h.Name.trim() : void 0) ?? "Unknown";
      const fromInternal = pickInternalNameFromHospital(h);
      const internalName = fromInternal ?? (typeof h.Name === "string" && h.Name.trim() !== "" ? h.Name.trim() : null);
      if (code === "Unknown") {
        const resolved2 = resolvedInternalConsultHospital();
        hospitalCache.set(id, resolved2);
        return resolved2;
      }
      const resolved = {
        code,
  ...
  try {
    ...
    const claimedColumnResult = await client.query(`
            SELECT column_name
            FROM information_schema.columns
            WHERE table_schema = 'public'
              AND table_name = $1
    ...
    const runFetchPhase = async (phaseLabel, phaseConstraints, sortField, startCursor) => {
      ...
      while (remaining > 0) {
        ...
        let response;
        try {
          response = await axios_default.get(bubbleApiUrl, {
            params: {
          ...
          if (error instanceof AxiosError2 && error.response?.status === 400) {
        ...
        for (const item of results) {
          ...
          if (!regionStr && regionId) {
            try {
              const rRes = await axios_default.get(
          ...
          const hospitalBubbleId = item["Primary care hospital"]?.trim() || item["Referred by hospital"]?.trim();
          const {
            code: hospitalCode,
      ...
      return { nextCursor: cursorStart, remaining, timedOut: timedOut2 };
    };
    let bubbleRemaining = 0;
    let nextCursorOut = initialCursor;
    let timedOut = false;
    let incrementalResumeOut;
...
var runIngestionPasses = async (bubbleToken, dbUrl, perPassMaxDurationMs, options = {}) => {
...
var ingestionTask = async () => {
  ...
  try {
    ...
    const perPassMs = Number.isFinite(configuredMaxDuration) ? Math.max(5e3, configuredMaxDuration) : 8e3;
    ...
    console.log("Scheduled ingestion summary:", summary);
...
var handler = schedule("*/5 * * * *", ingestionTask);
